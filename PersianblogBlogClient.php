...
class PersianblogBlogClient
{
    ...
    public function send_post($title, $body, $blog_tag = null){
        if ($blog_tag == null){
            $blog_tag = $this->blog_tag;
        }
        // first encode values
        $title = urlencode($title);
        $body = urlencode($body);
        // try to load posting page
        $ch = $this->grab_page_handle("http://persianblog.ir/CreatePost.aspx?blogID=$blog_tag");
    ...
    {
        // TODO: Implement submit_post() method.
    }
}